...
function deploymentOracleAddress() {
  const explicit = process.env.MM_ORACLE_ADDRESS ?? process.env.ORACLE_ADDRESS ?? process.env.NEXT_PUBLIC_ORACLE_ADDRESS;
  ...
  return String(deployment?.contracts?.oracle ?? deployment?.contracts?.storkOracleAdapter ?? "");
}
...
async function main() {
  ...
  if (network.chainId !== HORIZEN_TESTNET_CHAIN_ID) {
    throw new Error(`Refusing MM bot start: expected Horizen testnet ${HORIZEN_TESTNET_CHAIN_ID}, got ${network.chainId}`);
  }

  const [signer] = await ethers.getSigners();
  if (!signer) throw new Error("No signer configured for the MM bot");

  const oracleAddress = deploymentOracleAddress();
  if (!ethers.isAddress(oracleAddress)) {
    throw new Error("Set MM_ORACLE_ADDRESS or deploy Symbasis in demo oracle mode first");
  }
  ...
  while (true) {
    ...
    const updates = [
      {
        temporalNumericValue: {
          timestampNs,
          quantizedValue: ethers.parseUnits(eth.markPrice.toFixed(8), 18)
        },
        id: ETH_USD_FEED,
      ...
      },
      {
        temporalNumericValue: {
          timestampNs,
          quantizedValue: ethers.parseUnits(btc.markPrice.toFixed(8), 18)
        },
        id: BTC_USD_FEED,
        publisherMerkleRoot: ZERO32,
        valueComputeAlgHash: ZERO32,
    ...
    try {
      ...
      console.error(`MM oracle update failed: ${message}`);
    }
