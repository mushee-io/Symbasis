...
if (marketInput !== "ETH" && marketInput !== "BTC") {
  throw new Error("MM_MARKET must be ETH or BTC");
}
...
if (anchorInput !== undefined && (!Number.isFinite(anchorInput) || anchorInput <= 0)) {
  throw new Error("MM_ANCHOR_PRICE must be a positive number");
}
