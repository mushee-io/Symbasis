...
async function main() {
  ...
  if (network.chainId !== EXPECTED_CHAIN_ID) throw new Error(`Wrong chain: expected ${EXPECTED_CHAIN_ID}, got ${network.chainId}`);

  const deploymentPath = path.join(process.cwd(), "deployments", "horizen-testnet.json");
  if (!fs.existsSync(deploymentPath)) throw new Error("deployments/horizen-testnet.json not found. Deploy the stack first.");

  const deployment = JSON.parse(fs.readFileSync(deploymentPath, "utf8"));
  if (BigInt(deployment.chainId) !== EXPECTED_CHAIN_ID) throw new Error("Deployment artifact chain ID mismatch");
  const oracleMode = String(deployment.oracleMode ?? "stork").toLowerCase();
  if (oracleMode !== "demo" && oracleMode !== "stork") throw new Error(`Unknown oracle mode ${oracleMode}`);

  const c = deployment.contracts;
  for (const [label, address] of Object.entries(c as Record<string, string>)) await requireCode(address, label);
  if (oracleMode === "stork") await requireCode(EXPECTED_STORK, "Stork oracle");
  ...
  const oracleAddress = String(c.oracle ?? c.storkOracleAdapter);
  ...
  if ((await engine.oracle()).toLowerCase() !== oracleAddress.toLowerCase()) throw new Error("Engine oracle mismatch");

  if (oracleMode === "stork") {
    const oracle = await ethers.getContractAt("StorkOracleAdapter", oracleAddress);
    if ((await oracle.stork()).toLowerCase() !== EXPECTED_STORK.toLowerCase()) throw new Error("Oracle Stork address mismatch");
    if ((await oracle.owner()) === ethers.ZeroAddress) throw new Error("oracle has zero owner");
  } else {
    const oracle = await ethers.getContractAt("DemoPriceOracle", oracleAddress);
    const ethFeed = String(deployment.feeds.ETHUSD);
    const btcFeed = String(deployment.feeds.BTCUSD);
    const [ethPrice] = await oracle.latestPrice(ethFeed);
    const [btcPrice] = await oracle.latestPrice(btcFeed);
    if (ethPrice <= 0n || btcPrice <= 0n) throw new Error("Demo oracle is missing seeded prices");
    if ((await oracle.owner()) === ethers.ZeroAddress) throw new Error("demo oracle has zero owner");
  }
  ...
  for (const [label, contract] of [["vault", vault], ["registry", registry], ["engine", engine], ["intents", intents]] as const) {
  ...
  console.log(`oracleMode=${oracleMode}`);
