...
describe("DemoPriceOracle", function () {
  ...
  function update(feedId: string, price: bigint, timestampNs: bigint) {
    return [{
      temporalNumericValue: { timestampNs, quantizedValue: price },
      id: feedId,
  ...
  it("rejects public moves above five percent", async function () {
    ...
    const timestampNs = BigInt(block!.timestamp) * 1_000_000_000n;
    await expect(
      oracle.connect(trader).updatePrices(update(feedId, ethers.parseUnits("4000", 18), timestampNs))
    ).to.be.revertedWith("DEMO_STEP_TOO_LARGE");
  });
  ...
  it("rejects unknown feeds and nonzero update fees", async function () {
    ...
    const payload = update(unknown, ethers.parseUnits("100", 18), timestampNs);
  ...
  it("keeps feed configuration owner-only", async function () {
    const { trader, oracle } = await fixture();
    await expect(
      oracle.connect(trader).configureFeed(ethers.id("BTCUSD-DEMO"), ethers.parseUnits("110000", 18))
