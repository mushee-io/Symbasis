...
/// @dev Exposes the same frontend update surface as StorkOracleAdapter so the web app does not need a separate transaction path.
contract DemoPriceOracle is IPriceOracle, TwoStepOwnable {
    ...
    uint256 public constant MAX_PUBLIC_STEP_BPS = 500; // 5% per update
    ...
    /// @notice Compatibility method matching StorkOracleAdapter. Demo updates have zero oracle fee.
    function getUpdateFee(IStork.TemporalNumericValueInput[] calldata) external pure returns (uint256) {
        return 0;
    }

    /// @notice Permissionless bounded updates for testnet demos only.
    /// @dev Signature fields are intentionally ignored. Never use this contract with real funds.
